...
def run_script_two():
    ...
    console.see(tk.END)
    subprocess.run(["python3", "Populate.py"])
...
def run_script_three():
    ...
    console.see(tk.END)
    process = subprocess.Popen(["python3", "NeonCRM_Tool.py"], stdout=subprocess.PIPE, stderr=subprocess.STDOUT, bufsize=1, universal_newlines=True)
    while True:
